...
stock Cases_CheckDustReward(playerid, update_gui = 1)
{
	new cases_given = 0;
	...
	if(cases_given > 0) {
		new str[128];
		format(str, sizeof(str), "Поздравляем! Пыль конвертирована в ОСОБЫЙ КЕЙС (%d шт)!", cases_given);
		SendClientMessage(playerid, 0xFFFF00FF, str);
		
		if(update_gui && pCasesGUIOpen[playerid]) {
			Cases_UpdateGUI(playerid);
		}
		
		printf("[Cases] Player %d received %d special case(s) for dust", playerid, cases_given);
		return 1;
	}
	return 0;
}
...
stock Cases_TakeRewards(playerid, Node:json)
{
	...
	if(JSON_GetArray(json, "bt1", bt1Array) == 0) {
		...
		for(new i = 0; i < length; i++) {
			...
			if(JSON_ArrayObject(bt1Array, i, itemNode) == 0) {
				...
				if(rewardId > 0) Cases_GiveReward(playerid, rewardId);
			}
		}
	}
	...
	if(JSON_GetArray(json, "bt2", bt2Array) == 0) {
		...
		for(new i = 0; i < length; i++) {
			...
			if(JSON_ArrayObject(bt2Array, i, itemNode) == 0) {
				...
				JSON_GetInt(itemNode, "", rewardId);
				if(rewardId > 0) {
					totalDustGained += Cases_SprayReward(playerid, rewardId);
				}
			}
		}
	}
	...
	Cases_CheckDustReward(playerid, 0);
	...
	if(totalDustGained > 0) {
		new str[128];
		format(str, sizeof(str), "Получено пыли: %d. Текущий баланс: %d", totalDustGained, pCasesDust[playerid]);
		SendClientMessage(playerid, 0xFFFF00FF, str);
	}
...
stock Cases_GiveRewardInternal(playerid, caseIdx, awardIdx)
{
	...
	switch(rewardType) {
		case REWARD_TYPE_EXP: {
			...
			new str[128];
			format(str, sizeof(str), "Получено %d EXP! Выдано моментально", count);
			SendClientMessage(playerid, 0x00FF00FF, str);
		}
		case REWARD_TYPE_MONEY: {
			GivePlayerMoney(playerid, count);
			new str[128];
			format(str, sizeof(str), "Получено $%d! Выдано моментально", count);
			SendClientMessage(playerid, 0x00FF00FF, str);
		}
		case REWARD_TYPE_BC: {
			new str[128];
			format(str, sizeof(str), "Получено %d BC! (Добавь функцию выдачи в мод)", count);
			SendClientMessage(playerid, 0x00FF00FF, str);
		}
		case REWARD_TYPE_CASE: {
			pCasesCounts[playerid][internalId] += count;
			Cases_SavePlayer(playerid);
			new str[128];
			format(str, sizeof(str), "Получен кейс типа %d! Добавлен в рулетку", internalId);
			SendClientMessage(playerid, 0x00FF00FF, str);
		}
		case REWARD_TYPE_VEHICLE: {
			new str[128];
			format(str, sizeof(str), "Получен автомобиль (модель %d)! (Добавь функцию выдачи)", internalId);
			SendClientMessage(playerid, 0x00FF00FF, str);
		}
		case REWARD_TYPE_VIP: {
			new str[128];
			format(str, sizeof(str), "Получен VIP на %d часов! (Добавь функцию выдачи)", count);
			SendClientMessage(playerid, 0x00FF00FF, str);
		}
		case REWARD_TYPE_BP_EXP: {
			new str[128];
			format(str, sizeof(str), "Получено %d BP EXP! (Добавь функцию выдачи)", count);
			SendClientMessage(playerid, 0x00FF00FF, str);
		}
		case REWARD_TYPE_ITEM: {
			new str[128];
			format(str, sizeof(str), "Получен предмет (ID %d) x%d! (Добавь функцию выдачи)", internalId, count);
			SendClientMessage(playerid, 0x00FF00FF, str);
		}
		case REWARD_TYPE_DUST: {
			...
			Cases_SavePlayer(playerid);
			new str[128];
			format(str, sizeof(str), "Получено %d пыли! Зачислено на баланс", count);
			SendClientMessage(playerid, 0x00FF00FF, str);
		}
		case REWARD_TYPE_EVENT_RES: {
			new str[128];
			format(str, sizeof(str), "Получен ресурс события x%d! (Добавь функцию выдачи)", count);
			SendClientMessage(playerid, 0x00FF00FF, str);
		}
		default: {
			new str[64];
...
stock Cases_GetBonus(playerid, bonusId)
{
	...
	switch(bonusType) {
		case REWARD_TYPE_VEHICLE: {
			SendClientMessage(playerid, 0x00FF00FF, "Бонус: Получен автомобиль! (Добавь функцию выдачи)");
		}
		case REWARD_TYPE_DUST: {
			pCasesDust[playerid] += count;
			Cases_SavePlayer(playerid);
			new str[128];
			format(str, sizeof(str), "Бонус: Получено %d пыли! Зачислено на баланс", count);
			SendClientMessage(playerid, 0x00FF00FF, str);
		}
		case REWARD_TYPE_CASE: {
			pCasesCounts[playerid][internalId] += count;
			Cases_SavePlayer(playerid);
			new str[128];
			format(str, sizeof(str), "Бонус: Получено %d кейсов! Добавлено в меню", count);
			SendClientMessage(playerid, 0x00FF00FF, str);
		}
		case REWARD_TYPE_EVENT_RES: {
			SendClientMessage(playerid, 0x00FF00FF, "Бонус: Получен ресурс события! (Добавь функцию)");
		}
		case REWARD_TYPE_MONEY: {
			GivePlayerMoney(playerid, count);
			new str[128];
			format(str, sizeof(str), "Бонус: Получено $%d! Выдано моментально", count);
			SendClientMessage(playerid, 0x00FF00FF, str);
		}
		case REWARD_TYPE_BC: {
			new str[128];
			format(str, sizeof(str), "Бонус: Получено %d BC! (Добавь функцию выдачи)", count);
			SendClientMessage(playerid, 0x00FF00FF, str);
		}
		case REWARD_TYPE_VIP: {
			new str[128];
			format(str, sizeof(str), "Бонус: Получен VIP на %d часов! (Добавь функцию)", count);
			SendClientMessage(playerid, 0x00FF00FF, str);
		}
		case REWARD_TYPE_EXP: {
			SetPlayerScore(playerid, GetPlayerScore(playerid) + count);
			new str[128];
			format(str, sizeof(str), "Бонус: Получено %d EXP! Выдано моментально", count);
			SendClientMessage(playerid, 0x00FF00FF, str);
		}
		case REWARD_TYPE_BP_EXP: {
			new str[128];
			format(str, sizeof(str), "Бонус: Получено %d BP EXP! (Добавь функцию)", count);
			SendClientMessage(playerid, 0x00FF00FF, str);
		}
		case REWARD_TYPE_ITEM: {
			SendClientMessage(playerid, 0x00FF00FF, "Бонус: Получен предмет! (Добавь функцию выдачи)");
		}
	}
...
{
	if(GetPlayerAdminEx(playerid) < 6) return SendClientMessage(playerid, 0xFF0000FF, "Недостаточно прав!");

	extract params -> new to_player, amount; else return SendClientMessage(playerid, 0xCECECEFF, "Используйте: /givedust [id игрока] [количество]");

	if(!IsPlayerConnected(to_player) || !IsPlayerLogged(to_player))
		return SendClientMessage(playerid, 0xCECECEFF, "Такого игрока нет");

	if(!(1 <= amount <= 10000)) return SendClientMessage(playerid, 0xCECECEFF, "Можно выдавать от 1 до 10000 пыли за один раз");

	pCasesDust[to_player] += amount;

	new reason[144];
	format(reason, sizeof reason, "Администратор %s выдал Вам %d пыли", GetPlayerNameEx(playerid), amount);
	SendClientMessage(to_player, 0x00FF00FF, reason);

	format(reason, sizeof reason, "Вы выдали %d пыли игроку %s", amount, GetPlayerNameEx(to_player));
	SendClientMessage(playerid, 0x00FF00FF, reason);

	format(reason, sizeof reason, "[A] %s[%d] выдал пыль %s[%d] в кол-во %d, в итоге у игрока %d пыли",
	...
	format(reason, sizeof reason, "Выдал %s[acc:%d] %d пыли", GetPlayerNameEx(to_player), GetPlayerAccountID(to_player), amount);
	SendLog(playerid, LOG_TYPE_SUPERADMIN_ACTION, reason);

	return 1;
}

// Admin command to give cases
CMD:givecases(playerid, params[])
{
	if(GetPlayerAdminEx(playerid) < 6) return SendClientMessage(playerid, 0xFF0000FF, "Недостаточно прав!");

	extract params -> new to_player, case_id, amount; else return SendClientMessage(playerid, 0xCECECEFF, "Используйте: /givecases [id игрока] [id кейса] [количество]");

	if(!IsPlayerConnected(to_player) || !IsPlayerLogged(to_player))
		return SendClientMessage(playerid, 0xCECECEFF, "Такого игрока нет");

	new caseIdx = Cases_GetIndex(case_id);
	if(caseIdx == -1) return SendClientMessage(playerid, 0xCECECEFF, "Неверный ID кейса");

	if(!(1 <= amount <= 10000)) return SendClientMessage(playerid, 0xCECECEFF, "Можно выдавать от 1 до 10000 кейсов за один раз");

	pCasesCounts[to_player][caseIdx] += amount;

	new reason[144];
	format(reason, sizeof reason, "Администратор %s выдал Вам %d кейсов (ID %d)", GetPlayerNameEx(playerid), amount, case_id);
	SendClientMessage(to_player, 0x00FF00FF, reason);

	format(reason, sizeof reason, "Вы выдали %d кейсов (ID %d) игроку %s", amount, case_id, GetPlayerNameEx(to_player));
	SendClientMessage(playerid, 0x00FF00FF, reason);

	format(reason, sizeof reason, "[A] %s[%d] выдал кейсы %s[%d] в кол-во %d (ID %d), в итоге у игрока %d",
	GetPlayerNameEx(playerid), playerid, GetPlayerNameEx(to_player), to_player, amount, case_id, pCasesCounts[to_player][caseIdx]);
	SendMessageToAdmins(reason, 0x999999FF);

	format(reason, sizeof reason, "Выдал %s[acc:%d] %d кейсов (ID %d)", GetPlayerNameEx(to_player), GetPlayerAccountID(to_player), amount, case_id);
	SendLog(playerid, LOG_TYPE_SUPERADMIN_ACTION, reason);
...
CMD:mybc(playerid, params[])
{
	...
	format(str, sizeof(str), "BC: %d | Пыль: %d | Всего открыто: %d", GetPlayerDonateRub(playerid), pCasesDust[playerid], pCasesOpened[playerid]);
...
{
	if(GetPlayerAdminEx(playerid) < 1) return SendClientMessage(playerid, 0xFF0000FF, "Недостаточно прав!");
...
stock AddRewardToPlayerDB(playerid, type, item, amount)
{
	#pragma unused playerid, type, item, amount
	return 1;
}
